            ...
            if (!empty($value)) {
                ...
                if (empty($errors)) {
                    ...
                    if(isset($postData['token']) && !empty($postData['token'])){
                        if($showRecaptcha['enable_recaptcha'] == "yes" && !empty($recaptchaV3SiteKey) && !empty($recaptchaV3SecretKey)) {
                            $captchaStatus = 0;
                            ...
                            if ($get_recaptcha_response->success == true && $get_recaptcha_response->score >= 0.5 && $get_recaptcha_response->action == 'contact_form') {
                                $captchaStatus = 1;
                            }
                        }

                        // Google Recaptcha V2
                    } else if ($showRecaptcha['enable_recaptcha'] == "yes"  && !empty($recaptchaV2SiteKey) && !empty($recaptchaV2SecretKey)) {
                        $captchaStatus = 0;
                ...
                if ($value) {
                    ...
                    if (!empty($value['value']) || $slug == "contact_us" || (isset($value['is_agent']) && $value['is_agent'])) {
                        ...
                        if ($channel_type == "contact_us") {
                            ...
                            if($value['enable_recaptcha'] == "yes") {
                                if($value['captcha_type'] == "v2" && !empty($value['v2_site_key'])) {
                                    $recaptchaV2SiteKey = $value['v2_site_key'];
                                    $enableRecaptcha = 1;
                                }else if($value['captcha_type'] == "v3" && !empty($value['v3_site_key'])) {
                                    $recaptchaV3SiteKey = $value['v3_site_key'];
                                    $enableRecaptcha = 1;
                                }
                            }
        ...
        if (get_option('cht_active'.$index) && $is_traffic_source) {
            ...
            if ($page_status) {
                ...
                if ($len >= 1 && !empty($social)) {
                    ...
                    $setting['widget_fa_icon']     = get_option('widget-fa-icon'.$index);
