...
class Filters {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_coupon_data_store_cpt_get_coupons_query', [ $this, 'woocommerce_coupon_data_store_cpt_get_coupons_query' ], 10, 2 );
	}
	...
	public function set_args( $args ) {
		...
		if ( ! empty( $filter['discount_type'] ) ) {
			$args['meta_query'][] = [ 'meta_key' => 'discount_type', 'meta_value' => $filter['discount_type'] ];
		}

		if ( ! empty( $filter['has_expire_date'] ) && $filter['has_expire_date'] === 'yes' ) {
	...
	public function woocommerce_coupon_data_store_cpt_get_coupons_query( $wp_query_args, $query_vars ) {
	...
	public function add_filter_to_posts_join( $join ) {
		...
		if ( ! empty( $filter['has_expire_date'] ) && $filter['has_expire_date'] === 'no' ) {
			global $wpdb;
	...
	public function add_filter_to_posts_where( $where ) {
		...
		if ( ! empty( $filter['has_expire_date'] ) && $filter['has_expire_date'] === 'no' ) {
			$where .= " AND ( {$wpdb->postmeta}.meta_key = 'date_expires' AND {$wpdb->postmeta}.meta_value is null )";
		}
